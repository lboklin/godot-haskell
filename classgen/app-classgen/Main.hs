...
main = do
  ...
  -- print classes
  let state = execState (mapM_ addClass classes >> resolveMethods)
                        (ClassgenState mempty mempty mempty)
  mapM_ writeModule (HM.elems (state ^. modules))
  writeModule $ godotApiAuto (state ^. mainDecls)
 where
  godotApiAuto decls = Module ()
                              (Just $ classModuleHead $ classExports decls)
                              []
                              classImports
                              decls
  ...
  classExports decls =
    ExportSpecList ()
      $ tcMethod
      : tcHasBaseClass
      : mapMaybe fromNewtypeOnly decls
  tcMethod = EThingWith () (EWildcard () 0) (UnQual () (Ident () "Method")) []
  tcHasBaseClass =
    EThingWith () (EWildcard () 0) (UnQual () (Ident () "HasBaseClass")) []
...
writeModule mdl@(Module _ (Just (ModuleHead _ (ModuleName () name) _ _)) _ _ _)
  = do
